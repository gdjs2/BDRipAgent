...
def review(ctx, track_id=None):
    ...
    while True:
            ...
            with httpx.Client(
                ...
                with client.stream(
                    ...
                    if response.status_code == 409:
                        ...
                        return consume_stream(ctx, response, label=label)
        ...
        ctx.progress(99, phase=f"Waiting for {label.lower()}")
        for _ in range(5 * 2 ** min(failures, 2)):
            ctx.check()
            time.sleep(1)


def classify(ctx, track, source, *, require_confident=True, agent_review=False):
    ...
    ctx.progress(99, phase=f"Reading subtitle track {track_id}")
    buffer = ""
    ...
    ctx.run(
        [
            ...
            root,
            "--limit",
            str(behavior()["integrations"].get("subtitle_detection", {}).get("sample_cues", 96)),
            "--language",
            original["language"],
            "--sup2sup-source",
    ...
    if report["samples"] and (
        ...
        ctx.progress(99, phase=f"Reviewing subtitle track {track_id} with agent")
        ...
        try:
            ...
            if not preliminary.language_confident:
                for key in ("language", "script", "language_confident", "language_evidence"):
            ...
            raise
    language = "und"
    if decision.language_confident:
        try:
            language = detected_language(decision, original["language"])
        except ValueError as error:
            decision = decision.model_copy(
                update={
                    "language_confident": False,
                    "explanation": decision.explanation + " " + str(error),
                }
            )
