...
(async()=>{
 ...
 try {
  ...
  let count=15, best=15, running=false;
  const policy=()=>({best_count:count,count:7,representative:4,encode_challenging:3,min_spacing_seconds:30,min_timeline_bins:3,max_per_scene:1,policy:''});
  const job=()=>({id:'movie',title:'Screenshot review',year:2026,source_path:'Movie.mkv',analysis_profile:'x265-live',state:running?'SCREENSHOT_AGENT_SELECTION':'WAITING_FOR_SCREENSHOT_SELECTION',analysis:{},validation:{},tracks:[],artifacts:[],screenshot_policy:policy(),tasks:running?[{id:'review',type:'select_screenshots',status:'RUNNING',stage:'SCREENSHOT_AGENT_SELECTION',created_at:'2026-09-22T00:00:00Z',progress:0,progress_detail:{},attempt:1}]:[]});
  const gallery=()=>({candidates:40,shortlisted:40,recommended:best,final:0,items:Array.from({length:40},(_,i)=>({id:String(i+1),candidate_id:i+1,shortlisted:true,selected:false,info:{recommendation_rank:i<best?i+1:null,timeline_seconds:(i+1)*100,source_frame_number:(i+1)*2400,reason:'Character detail'},reservation:i===0?{codec:'x264',job_id:'other',frame_number:2400,spacing_seconds:30}:null}))});
  ...
  await page.route('**/api/**',async route=>{
   ...
   let body=[];
   if(path==='/api/config')body={profiles:{'x265-live':{codec:'x265'}},screenshots:{...policy(),best_count:20},stages:['ENCODING','SCREENSHOT_AGENT_SELECTION','WAITING_FOR_SCREENSHOT_SELECTION']};
  ...
  assert.equal(await page.getByRole('button',{name:'Best 15',exact:true}).count(),1,'Saving preserves current images');
  await input.fill('41');
  ...
  await page.getByText(/The agent is reviewing screenshots/).waitFor();
  assert.equal(requests.at(-1)[1].best_count,20);
